...
Static site that renders AI-generated stock reports via Jekyll. The “new” and “rerun” links on the Pages site just open the `generate-report` GitHub Actions workflow with prefilled inputs.
...
With the secrets in place, the new/rerun links on GitHub Pages will open a prefilled workflow page; click **Run workflow** to kick off `generate-report.yml`. The workflow installs dependencies, runs `stonks.py <TICKER>`, and commits the updated `_reports/` Markdown back to the default branch, triggering a Pages rebuild.
